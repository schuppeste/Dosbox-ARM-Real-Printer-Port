...
void CPrinter::gen_random(char *s, const int len) {
...
char filename[20]={};
char buffer[10]={};
gen_random(buffer,10);
sprintf (filename, "lpr -J\"%s\"",buffer);
		FILE* psfile = NULL;
		 if ((psfile = popen(filename,"w")) == NULL)
  {
...
//		psfile = (FILE*)outputHandle;
  ...
  if (pclose(psfile) == -1)
    ...
    printf("Report spooled successfully\n");
		...
		//fclose(psfile);
		 //cupsPrintFilecupsPrintFile(cupsGetDefault(), psfile, "cairo PS", 0, NULL);
	//		outputHandle = NULL;
	 //cupsPrintFile(cupsGetDefault(), "Page1.ps", "cairo PS", 0, NULL);
	//unlink( "Page1.ps" );
}
#else
		else if (strcasecmp(output, "ps") == 0)
...
void CPrinter::sprintASCII85(char* f, Bit16u b)
{
	if (b != 256)
	{
		if (b < 256)
